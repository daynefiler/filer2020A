...
## Load cached objects
rdbFls <- list.files("cache", pattern = ".rdb$", full.names = TRUE)
rdbFls <- sub(".rdb$", "", rdbFls)
sapply(rdbFls, lazyLoad, envir = globalenv())

## Map subjectMeta to subjectCorr
...
The variable read-depth in ES precludes the single-sample window-smoothing approaches successfully applied in GS data \cite{chiang:2009aa}\added{, e.g. Control-FREEC \cite{boeva:2012aa}, CONDEL \cite{yuan:2020aa}, CNV\_IFTV \cite{yuan:2019aa}, CNVnator \cite{abyzov:2011aa}, ERDS \cite{zhu:2012aa}}; therefore we must rely on comparative analysis for interrogating copy number.
...
Looking at the total mapped molecules, the IDT-MC pool had over double the relative standard deviation (\Sexpr{poolTbl[pool == "IDT-MC", rsdTotal]}\%) of any other pool.
...
\added{Again excluding the two samples, we looked at the single-exon calls; \Sexpr{callByVarLen[vl == 1 & prd == "mc" & tst == "wg", pct*100]}\% of mcCNV single-exon calls and \Sexpr{callByVarLen[vl == 1 & prd == "ed" & tst == "wg", pct*100]}\% of ExomeDepth single-exon calls overlapped with the genome calls.
...
Despite the benefits, many clinical laboratories do not employ a multiplexed capture protocol because multiplexing\added{ reduces capture efficiency \cite{trost:2018aa} and} requires waiting to fill a pool and may delay results.
...
\replaced{The sequencing depth in clinical exomes varies widely between efforts, with average depths in the Clinical Sequencing Exploratory Research (CSER) consortium ranging from 63-233x \cite{green:2016aa}; others have suggested an ideal depth of 120x for SNP/indel calling \cite{kim:2015ab}.}{Typically, exome sequencing targets 30-50x coverage to ensure most targets have sufficient coverage for accurate basepair calling.}
We demonstrate the need for \deleted{much }deeper sequencing if we wish to establish exon-level variants.

Additionally, we recognize the increased capture efficiency in hard-to-capture regions using independent captures\added{; multiplexing the capture step reduces the capture efficiency by 20-30\% \cite{trost:2018aa}}.
...
<<>>=
file.copy("figures-a0plot-1.pdf", "figure1.pdf", overwrite = TRUE)
@
...
<<>>=
stitchPlots(figLst = list(list(path = "figures-mnVrPlot-1.pdf", width = 0.5),
                          list(path = "figures-mnVrPlot-2.pdf", width = 0.5)),
            width = 6.68,
            height = 3.34,
            figname = "figure2.pdf")
@
...
<<>>=
l <- list(list(path = "figures-edSelection-1.pdf", width = 0.5),
          list(path = "figures-edSelection-2.pdf", width = 0.5, newline = TRUE),
          list(path = "figures-edSelection-3.pdf", width = 0.5),
          list(path = "figures-edSelection-4.pdf", width = 0.5))
stitchPlots(figLst = l,
            width = 6.68,
            height = 5,
            figname = "figure3.pdf")
@
...
<<>>=
l <- list(list(path = "figures-simPlts-1.pdf", width = 0.33),
          list(path = "figures-simPlts-2.pdf", width = 0.33),
          list(path = "figures-simPlts-3.pdf", width = 0.33, newline = TRUE),
          list(path = "figures-simPlts-4.pdf", width = 0.33),
          list(path = "figures-simPlts-5.pdf", width = 0.33),
          list(path = "figures-simPlts-6.pdf", width = 0.33, newline = TRUE))
stitchPlots(figLst = l,
            width = 6.68,
            height = 4.5,
            figname = "figure4.pdf")
...
<<>>=
l <- list(list(path = "figures-vennDiag-1.pdf", width = 0.5),
          list(path = "figures-vennDiag-2.pdf", width = 0.5, newline = TRUE),
          list(path = "figures-vennDiagLgnd-1.pdf", width = 1))
stitchPlots(figLst = l,
            width = 6.68,
            height = 3,
            figname = "figure5.pdf")
...
caption(poolTbl) <- "Summary of whole-exome sequencing. `pool' indicates the name of the pool of samples; `capture' indicates the capture platform for the pool; `N' gives the number of samples in the pool; `medExon' gives the pool median of the subject median mapped molecule count per exon; `medTotal' gives the median by pool of total mapped molecule counts per subject; `minTotal' and `maxTotal' give the minimum \\& maximum total mapped molecules; `rsdTotal' gives the relative standard deviation (SD/mean*100) of total mapped molecules. $\\dagger$ indicates captures were performed independently on each sample within the pool, otherwise captures were multiplexed across all samples within the pool."
label(poolTbl) <- "tab:poolSummary"
print(poolTbl)
@

<<wgsCallBySbjXtable, results='asis'>>=
...
<<predMetricsXtable, results='asis'>>=
...
predMetrics <- xtable(predMetrics, align = "ccllrrrrr")
