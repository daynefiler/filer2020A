##----------------------------------------------------------------------------##
## Script to perform ExomeDepth algorithm w/ best pars on varDepth simulation
##----------------------------------------------------------------------------##
...
odir <- file.path(wd, "varDepthEdBestCalls")
...
doCalc <- function(fl, dep, odir) {
  ...
  ofmt <- sub(".counts$", ".edBestCallSummary", ifl)
